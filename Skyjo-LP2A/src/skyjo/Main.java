...
public class Main {
	...
	public static void test(String[] args) {
		Deck deck = new Deck();
		BoardArray boards = new BoardArray(4,deck);
		...
		if(boards.get(0).checkColumn(0)) {
			boards.get(0).eraseColumn(0);
			boards.get(0).displayBoardSet();
	...
	public static void main(String[] args) {
		...
		while (!points.isHundred()) { // The game loop stop if someone has >= 100 points
			...
			while (!boards.isNotHidden()) {
				...
				for (int i = 0; i < nbPlayer; ++i) {					
					...
					while (!userPlayed) { // this loop stop when the player has play
						...
						case 1 :  
							page1(boards,discard,deck,i); // Display the player board
							myObj.nextLine();
						...
						case 4 : 
							...
							userSwitch = page4(boards,i,card); // Get the card choice of the player for the switch 
							discard.push(boards.get(i).switchCardBoard(userSwitch[0]-1, userSwitch[1]-1, card)); // Switch the card drawn and the choice of the player
							
							// Specific Rules
							if (boards.get(i).checkColumn(userSwitch[1]-1)) { // Implemant the rule where if all the card of a column are the same
								boards.get(i).eraseColumn(userSwitch[1]-1); // We erase the column
								return;
							}
							
							if (!alreadyFinished && boards.get(i).boardNotHidden()) { // If no one has alreay reveal his board entirely and the player is the first
								alreadyFinished = true; // Then someone has finish now
								playerFinishedFirst = i; // And we memorise that he was the first
							}
							// Specific Rules
						...
						case 5 :
							...
							discard.push(boards.get(i).switchCardBoard(userSwitch[0]-1, userSwitch[1]-1, card));
							userPlayed = true;
			...
			for (int i = 0; i < nbPlayer; i++) {
				pointsArray[i] = boards.get(i).calculatePoint();
			}
			...
			for (int i = 0; i < nbPlayer; i++) { // Increment the points made to the point manager
