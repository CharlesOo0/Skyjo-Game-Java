...
public class BoardSet {
	 ...
	 *
	 * Behavior :
	 * Iterate of the two dimensional board, increment every card value to a string.
	...
	public void displayBoardSet() {
		...
		for (int i = 0; i < rowSize; i++) { // Iterate over the line of our bidimensional array			
			output = new String(); // Reinitialize each line the string
			for (int j = 0; j < columnSize; j++) { // Iterate over the column of our bidimensional array
				
				if(this.board[i][j].getHidden()) { // If the card is hidden we print an "x"
					output += "  x  ";
				}else { // Else we print the card
					output += " " + getUV(this.board[i][j].getCard());
				}
	...
	protected String getUV(int cardValue) {
		Locale locale = new Locale("");		
		ResourceBundle testProp = ResourceBundle.getBundle("uv", locale);
		return testProp.getString("UV" + toString(cardValue));
	}
